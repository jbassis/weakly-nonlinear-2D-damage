...
class glenFlow(object):
    ...
    def cohes(self,strainFun,peff=None,functionSpace=None):
        if functionSpace == None:
            strain = strainFun
            tau_y = np.maximum(self.yield_strength+self.mu*peff -(self.yield_strength+self.mu*peff -self.yield_min)*strain/self.crit_strain,self.yield_min)
            #tau_y2 = np.maximum(self.yield_strength -(self.yield_strength -self.yield_min)*strain/self.crit_strain,self.yield_min)
            #tau_y = 2*(1/tau_y1+1/tau_y2)**(-1)
            #tau_y = np.minimum(tau_y1,tau_y2)
            assert(np.min(tau_y)>0)
        else:
            tau_y = Function(functionSpace)
            strain = interpolate(strainFun,functionSpace).vector().get_local()
            peff_f = interpolate(peff,functionSpace).vector().get_local()
            #tau_y = np.maximum(self.yield_strength+self.mu*peff_f -(self.yield_strength+self.mu*peff_f -self.yield_min)*strain/self.crit_strain,self.yield_min)
            #tau_y2 = np.maximum(self.yield_strength -(self.yield_strength -self.yield_min)*strain/self.crit_strain,self.yield_min)
            #tau_y.vector()[:] = 2*(1/tau_y1+1/tau_y2)**(-1)
            #tau_y.vector()[:] = np.minimum(tau_y1,tau_y2)
            tau_y.vector()[:] = np.maximum(self.yield_strength+self.mu*peff_f -(self.yield_strength+self.mu*peff_f -self.yield_min)*strain/self.crit_strain,self.yield_min)
            assert(np.min(tau_y.vector().get_local())>0)
        return tau_y

    def plastic_visc(self,epsII,strain,peff=None,functionSpace=None):
        """
        ...
        tau_y = self.cohes(strain,peff,functionSpace)
        if functionSpace ==None:
    ...
    def strain_update(self,epsII,temp,strain,peff,dt):
        """ Needs to be called at the particle level """

        eta_visc = self.ductile_visc(epsII,temp,functionSpace=None)
        eta_plas = self.plastic_visc(epsII,strain,peff,functionSpace=None)
    ...
    def __call__(self,epsII,temp,strain,peff=None,functionSpace=None):
        """
        Calculate effective viscosity
        Input: u_values: velocity fields (as Fenics array thingy)
               scalar : the appropriate FENICS function space we live in
