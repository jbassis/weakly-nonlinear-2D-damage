...
class Stokes2D:
   def __init__(self, mesh,visc_func,left_vel=0.0,right_vel=None):
       ...
       # Effective pressure, hidden variable
       self.p_k= interpolate(Constant(0.0),self.scalar)
   ...
   def markBoundaries(self):
       ...
       sea_level_offset = 10#-self.sea_level_offset
       class RightTop(SubDomain):
           ...
           def inside(self, x_values, on_boundary):
               ...
               return on_boundary and x_values[1] > (sea_level-sea_level_offset)
       GAMMA_2 = RightTop()
       GAMMA_2.mark(self.boundary_parts, 6,check_midpoint=False)
       ...
       if self.left_vel != None:
           class Left(SubDomain):
               ...
               def inside(self, x_values, on_boundary):
                   ...
                   return on_boundary and (x_values[0]-left_wall<10000000000*DOLFIN_EPS_LARGE)
       ...
       if self.right_vel != None:
           class Right(SubDomain):
               ...
               def inside(self, x_values, on_boundary):
                   ...
                   return on_boundary and (right_wall-x_values[0]<10000000000*DOLFIN_EPS_LARGE)
           GAMMA_4 = Right()
   ...
   def solve(self,p,dt=3600,tolerance=1e-6,relax_param=1.0):
       ...
       p_k = self.p_k
       peff = Function(Q)
       peff.vector()[:] = np.maximum(interpolate(p_k,Q).vector().get_local()- interpolate(self.water_pressure,Q).vector().get_local(),0)
       eta = visc(epsII ,temp,strain,peff,Q)
       ...
       tau_y = self.visc_func.cohes(strain,peff,Q)
       ...
       length = self.right_wall-self.left_wall
       strain_rate = (self.right_vel-self.left_vel)/length
       mu=self.visc_func.ductile_visc(strain_rate,-20+273)
       txx = 2*mu*strain_rate
	...
	#)
       ...
       while ((float(err_rel) > tolerance) and (count < maxit)):
           ...
           assign(p_k, p)
           peff_k = Function(Q)
           peff_k.vector()[:] = interpolate(p_k,Q).vector().get_local()- interpolate(self.water_pressure,Q).vector().get_local()
           assign(peff, peff_k)
           count += 1
       ...
       #peff = interpolate(p,Q) - interpolate(self.water_pressure,Q)
       self.eta=self.visc_func(epsII ,temp,strain,peff,Q)
       self.eta_visc=self.visc_func.ductile_visc(epsII ,temp,Q)
       self.eta_plas=self.visc_func.plastic_visc(epsII ,strain,peff,Q)
       #self.epsII = epsII
   ...
   def update(self,u,dt,p,remesh=True,remesh_elastic=False):
       ...
       if self.method==1:
           ...
           deps_dt.vector()[:] = self.visc_func.strain_update(epsII.vector().get_local(),temp.vector().get_local(),strain.vector().get_local(),peff.vector().get_local(),dt_m)
           deps_dt_eff = Function(Vdg)
           #if self.deps_dt == None:
           p.interpolate(deps_dt,1)
           pstrain_new = np.minimum(np.maximum(p. return_property(mesh , 1) + pstrain,0.0),0.1)
       ...
       if remesh == True:
           if remesh_elastic==False:
               ...
               if self.calving_front == False:
                  ...
                  self.markBoundaries()
                  # Now remesh
                  new_mesh=self.mesh.remesh(max_length=10*self.mesh.length)
                  print('Finished remeshing')
               ...
               print("updating mesh coordinate in line 846") 
       ...
       if self.u_k!=None:
           ...
           self.p_k = interpolate(self.p_k,self.scalar)
   ...
   def remesh_elastic(self,vel,dt):
       ...
       bc2.apply(uq.vector())
       # Comment for anything other than ice shelf!!!!!!
       #print("Warning: COMMENT LINES BELOW")
       #bc3 = DirichletBC(Q.sub(0), Constant(self.right_vel), self.boundary_parts,11)
       #bc3.apply(uq.vector())
   ...
   def remesh(self,u,dt):
       ...
       bcTop = DirichletBC(Q, 12, self.boundary_parts, 6)
       ...
       #filter = (xdof[bot_nodes]<self.right_wall) & (xdof[bot_nodes]>self.left_wall)             
       xbot=xdof[bot_nodes]#[filter]
       zbot=zdof[bot_nodes]#[filter]
       length = np.max(xbot)-np.min(xbot)
       # Extract velocity components at bottom of the mesh
       ux_bot = ux[bot_nodes]#-self.strain_rate*(xbot-length/2)
       uz_bot = uz[bot_nodes]#+self.strain_rate*zbot#[filter]
       #b = -np.mean(uz_bot)
       bot_mean_init = np.mean(zbot)

       # Update bottom nodes
       xbot+=ux_bot*dt
       zbot+=uz_bot*dt+self.bot_melt*dt#*self.rho_i/self.rho_w
       ...
       xtop=xdof[top_nodes]
       ztop=zdof[top_nodes]
       length = np.max(xtop)-np.min(xtop) 
       # Extract velocity components at top of the mesh
       ux_top = ux[top_nodes]#-self.strain_rate*(xtop-length/2)
       uz_top = uz[top_nodes]#+self.strain_rate*(ztop)
       top_mean_init=np.mean(ztop)
       

       # Update top nodes
       xtop+=ux_top*dt
       ztop+=uz_top*dt#-self.bot_melt*dt*(1-self.rho_i/self.rho_w)   #-a*dt# - net_thinning*dt
       
       # Create interpolation function to define top and bottom of the ice
       id1 = np.argsort(xbot)
       id2 = np.argsort(xtop)
       #print("Max xbot sorted",max(xbot[id1]))
       #length = np.max(xbot)-np.min(xbot)
       xbot_sorted = xbot[id1]
       #xbot_sorted = np.mod(xbot[id1],total_length)
       zbot_sorted = zbot[id1]
       #xsurf_sorted = np.mod(xtop[id2],total_length)
       xsurf_sorted = xtop[id2]
       zsurf_sorted = ztop[id2]

       self.xsurf=xsurf_sorted
       self.zsurf=zsurf_sorted
       #filter1 = (xbot_sorted<length) & (xbot_sorted>0)
       #filter2 = (xsurf_sorted<length) & (xsurf_sorted>0)
       
       xb = np.hstack((xbot_sorted-length,xbot_sorted,xbot_sorted+length))
       zb = np.hstack((zbot_sorted,zbot_sorted,zbot_sorted))
       xs = np.hstack((xsurf_sorted-length,xsurf_sorted,xsurf_sorted+length))
       zs = np.hstack((zsurf_sorted,zsurf_sorted,zsurf_sorted))

       xb = np.hstack((xbot_sorted))
       zb = np.hstack((zbot_sorted))
       xs = np.hstack((xsurf_sorted))
       zs = np.hstack((zsurf_sorted))

       #zs = np.maximum(zs,0.0)
       bot_fun= interp1d(xb,zb,kind='linear',fill_value="extrapolate")
       surf_fun = interp1d(xs,zs,kind='linear',fill_value="extrapolate")
       #bot_fun= interp1d(xbot[id1],zbot[id1],kind='nearest',fill_value="extrapolate")
       #surf_fun = interp1d(xtop[id2],ztop[id2],fill_value="extrapolate")
       #surf_fun = interp1d(xtop[id2],ztop[id2],kind='linear',fill_value="extrapolate")
       self.xtop = xtop[id2]
       self.ztop = ztop[id2]
       self.xbot = xbot[id1]
       self.zbot = zbot[id1]
       ...
       mean_thick_new = np.mean(ztop)-np.mean(zbot)
       mean_thick_old = top_mean_init-bot_mean_init
       dz = (self.mesh.dz*mean_thick_new/mean_thick_old)
       dz = (self.mesh.dz)
       #print(dz)
       new_mesh = meshModel.MeshModelPoly(surf_fun,bot_fun,bed_fun,Nx=Nx,Nz=Nz,length=length,dz=dz)
       #new_mesh = meshModel.MeshModelPoly(surf_fun,bot_fun,bed_fun,Nx=Nx,Nz=Nz,length=length)
